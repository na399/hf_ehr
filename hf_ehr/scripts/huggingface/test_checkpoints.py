...
def list_matching_files(dir, ckpt_val: int = '*'):
    ...
    for model in ['bert-base', 'gpt-base', 'hyena-large', 'mamba-tiny', 'llama-base']:
        ...
        output += os.popen(f'ls ./data/suhana/hf_ehr/cache/runs_backup/{model}-*--clmbr/ckpts/train-tokens-total_nonPAD-true_val=*-ckpt_val={ckpt_val}-persist.ckpt').read().splitlines()
    # CLMBR-k
    output += os.popen(f'ls ./data/suhana/hf_ehr/cache/runs_backup/gpt-base-*--clmbr_*k/ckpts/train-tokens-total_nonPAD-true_val=*-ckpt_val={ckpt_val}-persist.ckpt').read().splitlines()
    return output
...
def get_max_token_ckpts():
    ...
    for model in ['bert-base', 'gpt-base', 'hyena-large', 'mamba-tiny', 'llama-base']:
        print("Searching for", model)
        output += os.popen(f'ls ./data/suhana/hf_ehr/cache/runs_backup/{model}-*--clmbr/ckpts/train-tokens-total_nonPAD-ckpt_val=*-persist.ckpt').read().splitlines()
    # CLMBR-k
    output += os.popen(f'ls ./data/suhana/hf_ehr/cache/runs_backup/gpt-base-*--clmbr_*k/ckpts/train-tokens-total_nonPAD-ckpt_val=*-persist.ckpt').read().splitlines()
    ...
    for file_path in output:
        # From 'train-tokens-total_nonPAD-ckpt_val=1000000000-persist.ckpt' to 1000000000
        ckpt_val: int = int(re.search(r'ckpt_val=(\d+)', file_path).group(1))
        # Part [MODEL] from './data/suhana/hf_ehr/cache/runs_backup/[MODEL]/ckpts/train-tokens-total_nonPAD-ckpt_val=*-persist.ckpt'
...
paths = list_matching_files('./data/suhana/hf_ehr/cache/runs_backup')
