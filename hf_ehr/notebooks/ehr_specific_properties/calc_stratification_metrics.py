...
if __name__ == '__main__':
    ...
    if not os.path.exists(path_to_output_file.replace('.parquet', '__metrics.parquet')):
        if not os.path.exists(path_to_output_file):
            print(f'Calculating inter-event times for {LABELING_FUNCTION}...')
            df = calc_inter_event_times(femr_db, pids, label_times, pids_idx)
        ...
        print(f'Already calculated inter-event time metrics for {LABELING_FUNCTION}...')
    ...
    if not os.path.exists(path_to_output_file.replace('.parquet', '__metrics.parquet')):
        if not os.path.exists(path_to_output_file):
            ...
            df = calc_n_gram_count(femr_db, pids, label_times, pids_idx)
            df.to_parquet(path_to_output_file)
            print(f'Loaded df.shape={df.shape}')
        else:
        ...
        print(f'Already calculated n-gram metrics for {LABELING_FUNCTION}...')
    ...
    if True or not os.path.exists(path_to_output_file.replace('.parquet', '__metrics.parquet')):
        # Metric 1: number of tokens
        PAD_TOKEN_ID: int = 4
        n_tokens = (timelines != PAD_TOKEN_ID).sum(axis=1)
        n_unique_tokens = [ len(set([ x for x in t if x != PAD_TOKEN_ID ])) for t in timelines ]
        ...
        for idx, pid in enumerate(pids):
            ...
            for e in femr_db[pid].events:
                if label_times is not None and e.start > label_times[idx]:
                    ...
                    break
                n_event += 1
                unique_events.add(e.code)
            n_events.append(n_event)
            n_unique_events.append(len(unique_events))
        ...
        print(f'Already calculated timeline length metrics for {LABELING_FUNCTION}...')
